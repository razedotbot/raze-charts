...
import { resolveTimezone } from "../../core/context";
...
/** The display zone the corner caption names (the same rule the tick labels use). */
export function displayTimezone(context: Pick<ChartContext, "timezone" | "symbolInfo" | "options">): string {
  const setting = context.timezone !== undefined ? context.timezone : (context.options?.timezone ?? null);
  return resolveTimezone(setting, context.symbolInfo);
}
 ...
 * TradingView. The joined half extends under the tag, which paints on top.
 */
export function drawBarCountdown(ctx: CanvasRenderingContext2D, v: FinanceView): void {
  ...
  const joinTop = box.below ? rect.y - TAG_H / 2 : rect.y;
  roundRect(ctx, rect.x, joinTop, rect.w, rect.h + TAG_H / 2, 3);
  ctx.fill();
  let size = 10;
  ctx.font = `${size}px ${v.fontFamily}`;
