...
import { clipToRect, lineStart, withBitmapSpace, type BitmapSpace } from "./pixel";
import type { FinanceView } from "./view";

/** Main-series line width in CSS pixels, rounded to whole device pixels. */
...
export function drawLineArea(ctx: CanvasRenderingContext2D, v: FinanceView, fill: boolean): void {
  ...
  withBitmapSpace(ctx, v.dpr, (s) => {
    ...
    const lw = Math.max(1, Math.round(LINE_WIDTH * Math.min(s.hpr, s.vpr)));
