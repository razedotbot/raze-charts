...
| Heatmap | **Yes** | Standalone composition with configured band-domain layout, a diverging color scale, square cells, and labels where the whole value fits. Values print as plain numbers by default; `valueFormat` selects `"percent"`, `"signed"`, `"signed-percent"` or a function for cells, tooltips and the colour bar. |
| Axis tick precision and number formats | **Yes** | Tick decimals come from the tick step (a 0.0005 step reads 1.0850, 1.0855); value labels use the series' data precision (at most 8 decimals). Magnitudes from a million up use compact K/M/B/T notation. `scales.*.tickFormat` overrides both. |
...
| Calendar time axis | **Yes** | Time scales tick on UTC calendar boundaries from 1 second to centuries (1/5/15/30 s and min, 1/3/6/12 h, days, weeks, months, quarters, years). Labels switch between `HH:mm`, `D Mon`, `Mon` and `YYYY`, boundary ticks name the higher unit, and tooltips add the year or clock when the data need it. Display time zones other than UTC are not configurable on `/chart`. |
