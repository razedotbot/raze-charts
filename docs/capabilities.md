...
| Overlays in element fullscreen | **Yes** | Every menu (Indicators, chart type, objects tree, symbol results, the context menu), bottom sheet and kit overlay renders inside the fullscreen element while the chart is fullscreen, and an open one moves with the chart when fullscreen starts or ends. They also follow the chart into (open) shadow roots, where presses, arrow keys, Escape and focus work as in the light DOM. |
| Popup menus | **Yes** | `openPopup()` menus flip to the side of their anchor with room (`below-start` opens above an anchor near the bottom, `right-start` to the left of one at the right edge; a context menu flips around the cursor), cap their height at the viewport minus 16px and scroll inside with contained overscroll, and follow their anchor through resizes, scrolls, content changes and fullscreen changes. Row hover, focus, selection and disabled states are CSS with a single highlight that follows the keyboard and the mouse, and every menu shares one 4px row inset (`--raze-popup-inset`). `popupSeparator()` adds a divider the arrow keys skip. Menus fade and scale in over 120ms, with no motion under `prefers-reduced-motion`. Tab and Shift+Tab close a menu and continue from its button (menu-button pattern). |
| `onContextMenu` items | **Yes** | TradingView conventions: `{ text: "-" }` is a separator (redundant ones are dropped) and `{ text: "-Label" }` removes the default item named `Label`. The chart has no default items yet, so a removal only warns once and never renders. `position` groups items before (`"top"`, the default) or after (`"bottom"`) the defaults. An item without `click` is shown disabled with a warning; a throwing `click` is reported with `console.error`. |
