...
| Tooltip and crosshair | **Yes** | Pointer interaction in `mountChart`; native default is enabled unless set to `false`. `onTooltip`/`onSelect` payloads are data-space: `x` is a number on quantitative axes and the category on band axes, and samples carry `datum`, `index`, `markIndex`, and `seriesId`. Crosshair chips show the hovered datum, not the nearest tick. The tooltip stays under a stationary pointer across `update()`, resizes, and viewport changes; those repaints call `onTooltip` only when the hovered values change, and never from a repaint that `onTooltip` itself started. Overlays map through the plotted stage, so presets, the navigator, and CSS scaling do not offset them. |
...
| Retained scene diffing | **No** | `update()` preserves the mount, then recompiles and repaints the scene. It also re-reads the full data behind the navigator and zoom bounds, so rows mutated in place are picked up; a call that only moves the viewport reuses them unless rows were added or removed. |
