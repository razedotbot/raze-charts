...
| Gapped market sessions | **Yes** | `TimeIndex` maps actual timestamps onto adjacent logical bar indices; `session_breaks` draws optional lines at session opens (after a gap) and, for round-the-clock intraday data, at local midnight in the display zone. |
...
| Timezone label and bar countdown | **Yes** | `timezone_display` and `countdown` features (on by default). |
