...
| Listener errors | **Yes** | A throwing widget-event or subscription listener is logged as `[raze-charts] <event> listener threw`, delivered to `error` listeners as `{ code: "listener_threw", event, cause }`, and never stops the other listeners. |
| `executeActionById` / `getCheckableActionState` | **Subset** | Typed `ChartActionId`: `undo`, `redo`, `chartReset`, `timeScaleReset`, `insertIndicator`, `symbolSearch`, `paneObjectTree` (`objects_tree`), `stayInDrawingModeAction` (`stay_in_drawing_mode`), `magnet`, `hideAllDrawingTools`, `paneRemoveAllStudiesDrawingTools`, `volume_pane`. Other ids, such as `chartProperties`, throw with the supported list. `getCheckableActionState` reports `magnet`, `stayInDrawingModeAction` and `hideAllDrawingTools`. |
| `createStudy` arguments | **Subset** | Inputs as an object (booleans included, TradingView `in_<n>` ids mapped by position) or a positional array mapped onto the declared inputs (`[30]` is `length: 30`); malformed inputs reject with a TypeError. Overrides honour the primary plot colour (`plot.color`); options honour `disableUndo` and `priceScale: "as-series"`. Other override keys and options warn once. `studies_overrides` supplies the primary colour and input defaults for studies created through the API (`"moving average exponential.plot.color"`); keys matching no study, such as `volume.*`, warn at construction. |
