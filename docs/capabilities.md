...
| Study labels | **Yes** | `StudyDefinition.shortTitle` and `formatLabel(inputs)` customise the label shared by the legend, objects tree and Indicators menu. By default a label lists the numeric `defaults` inputs, or only the title when `compute` ignores its inputs argument. |
