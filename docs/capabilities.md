...
| Drawing tool registry | **Yes** | `defineDrawingTool()` registers host tools on the same contract as the built-ins: paint, hit-test, anchor count, property schema and defaults, undo and save/load. Invalid definitions throw, and unregistered kinds warn once. Runtime tools have no sidebar button yet. See [drawings](drawings.md). |
| Drawing styles | **Yes** | TradingView `linestyle` 0-4 and `linewidth` on every kind and the draft. Rectangle `backgroundColor`/`fillBackground`/`transparency`. Text font size, bold, italic, colour, background, border, `
` and word wrap. Fib follows TradingView's direction (level 0 at the second anchor; `reverse` flips it). The measure label uses the symbol's price formatter and shows `<n> bars, <span>`. Colours come from theme tokens (`drawings.*` overrides). |
