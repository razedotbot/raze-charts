...
| Drawing tool registry | **Yes** | `defineDrawingTool()` registers host tools on the same contract as the built-ins: paint, hit-test, anchor count, property schema and defaults, undo and save/load. Invalid definitions throw, `{ replace: true }` swaps a host tool during development reloads, and unregistered kinds warn once. Runtime tools have no sidebar button yet. See [drawings](drawings.md). |
