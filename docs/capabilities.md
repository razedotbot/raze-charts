...
| Left-toolbar tooltips and icons | **Yes** | Built-in sidebar buttons use one 18px, 1.5-stroke icon set and a themed kit tooltip instead of `title`: 500ms hover, keyboard focus or touch long-press, placed right of the toolbar, with the shortcut where one exists (Fit content: F). Custom `SidebarCustomItem` buttons get the same tooltip from their `title`. Menu rows keep a fixed check slot, so labels align whether checked or not. The header, scale bar and canvas trading-line hover still use native `title` tooltips. |
