...
| Widget events | **Subset** | `subscribe` / `unsubscribe` accept `drawing_event`, `trading_event` and `error`, typed by `WidgetEventMap`. Any other name, including TradingView events that never fire here (`onTick`, `study_event`, `onAutoSaveNeeded`, ...), throws a TypeError listing the supported events. |
