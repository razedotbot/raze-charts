...
| Heatmap | **Yes** | Standalone composition with configured band-domain layout, a diverging color scale, square cells, and labels where the whole value fits. Values print as plain numbers by default; `valueFormat` selects `"percent"`, `"signed"`, `"signed-percent"` or a function for cells, tooltips and the colour bar. Dense row and column labels thin; the crosshair chips still name the hovered row and column in full. |
