...
| Zero and negative prices | **Subset** | Candles, bars, line, area, baseline, columns and the last-price label place any finite price on a linear or percent scale, including lows and closes at or below zero; a log scale skips prices it cannot place, and non-finite OHLC values are whitespace. Price autoscale still assumes positive prices, so in the widget a symbol whose prices are all at or below zero renders an empty chart and data that crosses zero is cut off at 0. Do not ship negative-price symbols until autoscale supports them. |
