...
| Compare / multiple symbols | **Yes** | `createCompare(symbol)` overlays extra series and rejects, adding nothing, when the symbol cannot be resolved or its history fails. Compares refetch on interval and symbol changes and `resetData()`, page back with the main series, stream live bars under their own listener GUIDs, and unsubscribe on `removeEntity()` or `remove()`. A failed compare reload is reported once and retried on the next interval or symbol change or `resetData()`. The first compare switches the price scale to percent with autoscale; removing the last one restores the previous mode unless the user changed it, and the manual price range it cleared when nobody touched the scale and the main symbol and interval are unchanged. `raze.layout` `"2x1"` / `"2x2"` syncs range and crosshair. |
| Save/load chart layouts | **Yes** | Versioned JSON with stable drawing/study IDs via `save()` / `load()`; `disableSave` excludes a drawing and live broker/trading state is intentionally rehydrated separately. `load()` rejects, keeping the committed chart, when a saved compare symbol can no longer be resolved. |
